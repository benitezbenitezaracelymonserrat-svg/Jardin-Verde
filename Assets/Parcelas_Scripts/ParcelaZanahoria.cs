...
using UnityEngine;
public class ParcelaZanahoria : MonoBehaviour, IParcela, IParcelaConsultable
{
    ...
    public float tiempoMadurar = 8f;
    [Header("Efecto de agua")]
    public GameObject efectoAguaPrefab;
    private int[] estados; // 0 = vacio, 1 = semilla (esperando riego), 2 = regado/creciendo, 3 = maduro
    private bool[] mostrandoCaja;
    void Start()
    {
        estados = new int[semillas.Length];
        mostrandoCaja = new bool[semillas.Length];
        ZonaCosechaNivel2.Preparar(this, cajas, "Zanahoria");
        ActualizarTodo();
    }
    void ActualizarTodo()
    {
    ...
    {
        if (semillas[i]) semillas[i].SetActive(estados[i] == 1 || estados[i] == 2);
    ...
    public bool InteractuarSlot(int i, string herramienta)
    {
        ...
        else if (herramienta == "canasta" && estados[i] == 3)
        {
            ...
            Debug.Log("Cosechaste 1 zanahoria!");
            return true;
        }
        return false;
    }
    ...
    {
        yield return new WaitForSeconds(tiempoMadurar);
        estados[i] = 3;
        ActualizarVisual(i);
    }
}
